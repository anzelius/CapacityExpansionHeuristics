...
function recalculate_variables(params, results; type, power, e)
    ...
    if typeof(results.Power_production) <: JuMP.Containers.SparseAxisArray
        Discharge, Head, Eff_discharge, Power_production, Forebay_level =
            value.(results.Discharge), value.(results.Head), value.(results.Eff_discharge), value.(results.Power_production), value.(results.Forebay_level)
    else
    ...
    return Eff_discharge, Power_production, Profit, Forebay_level, Head
end
