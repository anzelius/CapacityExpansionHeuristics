...
strict_order = true 
order_grouping = :percentiles  # [:percentiles, :step] 
...
function initialize_run(river::Symbol, start_datetime::String, end_datetime::String, 
    ...
    expansion_strategy::String, order_metric::String, strict_order::Bool, order_grouping::Symbol, order_basis::Symbol; 
    ...
    elseif river in rivers
        _, plants_to_expand = identify_expansions_one_river(river, expansion_strategy) 
    else
        @error("Invalid river")
    end 
    ...
    results = run_model(order_of_expansion, start_datetime, end_datetime, objective, model, environmental_constraints_scenario, 
    price_profile_scenario, theoretical, save_file_name, recalc, save_variables, silent)  

    # these might not be able to be here, but is best suited here... 
    #printbasicresults(results) 
